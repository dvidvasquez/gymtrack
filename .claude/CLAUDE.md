...
- **Fase 9 — Registro manual desde Home (COMPLETADA):** el botón grande del footer (acción por default de `AppShell`) deja de ir directo a escanear; ahora abre un selector entre "Escanear QR" (flujo existente) y "Elegir de la lista" (flujo existente de `/exercises`, para ejercicios sin QR).
- **Fase 10 (futura, no MVP) — Offline-first:** guardado local con IndexedDB y sincronización en segundo plano cuando el usuario recupera conexión. No implementar hasta que se indique explícitamente.
...
- **`/log` (Fase 9) es el selector de método de registro, no un entry point de logueo en sí.** Es la nueva acción por default del footer de `AppShell` (antes era `/scan` directo) — `pages/LogEntryPage.tsx` solo ofrece dos `LinkButton` hacia flujos que ya existían (`/scan` y `/exercises`), no duplica nada del formulario de registro. `FooterActionContext`'s `FooterActionKind` renombró `'scan'` a `'register'` (ícono `IconBarbell`, ya no `IconQrcode`) porque la acción por default dejó de ser específicamente "escanear". `ScanPage` sigue siendo accesible directo (no se le sacó nada) y conserva su propio link a `/exercises` para quien ya está ahí y no tiene QR.
...
## Fase 9 — Registro manual desde Home (completada)

Motivada porque el botón grande del footer (acción por default de `AppShell`) llevaba directo a escanear, sin ofrecer el flujo manual (elegir de `/exercises`) como alternativa igual de accesible desde Home — había que entrar a Scan primero y ahí sí ver el link chico a "elegir de la lista". Ver "Decisiones de arquitectura" arriba para el detalle de `/log` como selector.

Implementado:
- `pages/LogEntryPage.tsx` (nueva, ruta `/log`): dos `LinkButton` hacia `/scan` y `/exercises`, ninguno de los dos flujos se tocó.
- `components/FooterActionContext.tsx`: `FooterActionKind` `'scan'`→`'register'`. `components/AppShell.tsx`: `DEFAULT_ACTION` ahora apunta a `/log` (ícono `IconBarbell`, label "Registrar ejercicio") en vez de `/scan` directo; también el fallback `action.to ?? '/scan'` pasó a `'/log'`.
- `router/AppRouter.tsx`: ruta `/log` nueva (sin parámetro, distinta de `/log/:qrCode` y `/log/exercise/:exerciseId`, sin ambigüedad de matching).
- `pages/ExercisesPage.tsx`: barra de búsqueda en tiempo real arriba de la lista (`Input` con ícono `IconSearch`), filtra client-side por `name` o `muscleGroup` (case-insensitive, substring) — sin debounce ni query a Supabase: la lista ya está completa en memoria (`useExercises`) y la cantidad de ejercicios de un gimnasio no justifica ir al servidor por cada tecla. Estado vacío distinto de "no hay ejercicios cargados": si hay ejercicios pero ninguno matchea la búsqueda, muestra `Ningún ejercicio coincide con "..."`.

Verificado con `tsc`, `oxlint` y `vite build` limpios, y con Playwright (rutas temporales): desde Home el footer ahora muestra aria-label "Registrar ejercicio" y lleva a `/log`, que renderiza las dos opciones; confirmado que `/log` es una ruta real protegida (rebota a `/login` sin sesión, igual que el resto de la app); la barra de búsqueda de `/exercises` renderiza, es tipeable y actualiza su valor en tiempo real (no se pudo probar el filtrado contra datos reales sin sesión autenticada — RLS bloquea el `select` para usuarios anónimos).

**Fase actual: ninguna — Fases 0 a 9 completas.** La Fase 10 (Offline-first) es futura y no se arranca sin pedido explícito del usuario; no asumir que "seguir con la siguiente fase" significa esa.
